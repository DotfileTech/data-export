...
# Configuration
API_URL = "https://api.dotfile.com/v1/individuals"
API_KEY = os.getenv("API_KEY")
DATE_FILTER = "2024-12-10T00:00:00Z"  # Replace with last sync date
OUTPUT_CSV = "outputs/individuals.csv"
...
def fetch_individuals():
    ...
    while True:
        ...
        if response.status_code == 200:
            data = response.json()
            individuals.extend(data['data'])  # Extract the list of individuals from the 'data' field
            pagination = data.get('pagination', {})
            if len(data['data']) < limit or pagination.get('count', 0) <= len(individuals):
                break
            page += 1
...
def save_to_csv(individuals):
    df = pd.json_normalize(individuals)  # Flatten nested JSON data
    df.to_csv(OUTPUT_CSV, index=False)
    print(f"Data saved to {OUTPUT_CSV}")

def main():
    individuals = fetch_individuals()
    detailed_individuals = []

    for case in individuals:
        case_id = case['id']
        details = fetch_case_details(case_id)
        if details:
            detailed_individuals.append(details)

    save_to_csv(detailed_individuals)
